...
class ClockOverlay:
    def __init__(self, config):
        self._cfg = config.overlays['clock']

    def draw(self, image: Image.Image) -> Image.Image:
        cfg = self._cfg
        now = datetime.now()
        time_str = now.strftime(cfg['time_format'])
        date_str = now.strftime(cfg['date_format']) if cfg.get('show_date') else ''

        lines = [time_str]
        if date_str:
            lines.append(date_str)

        font_large = get_font(cfg['font_size'])
        font_small = get_font(max(20, cfg['font_size'] - 14))
